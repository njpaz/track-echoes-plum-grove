...
function FishItem({ handleSetUserFishes, userFishes, fish }: { handleSetUserFishes: (fishName: string, target: HTMLElement) => void, userFishes: string[], fish: FishDataType }) {
  const hasFish = useMemo(() => userFishes.indexOf(fish.name) !== -1, [userFishes]);

  return (
    <div className={clsx('card', 'pt-2', 'cursor-pointer', hasFish && 'selected')} role="button" onClick={(evt) => handleSetUserFishes(fish.name, evt.target as HTMLElement)}>
...
export default function FishCards() {
  ...
  return fishData.map((fish: FishDataType) => <FishItem handleSetUserFishes={handleSetUserFishes} userFishes={userFishes} fish={fish} key={fish.link} />);
}
